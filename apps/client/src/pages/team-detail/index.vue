<template>
  <main v-if="currentView === 'overview'" :class="styles.page">
    <header :class="styles.header">
      <h1 :class="styles.logo">精弘毅行</h1>
    </header>
...
import TeamMemberDetailPopup from "./components/team-member-detail-popup/index.vue";
import TeamMemberList from "./components/team-member-list/index.vue";
...
const currentView = ref<TeamDetailView>("overview");
...
const refreshTeamData = async () => {
  await Promise.all([
    queryClient.invalidateQueries({ queryKey: [CLIENT_QUERY_KEY.TEAM.OVERVIEW] }),
...
>({
  ...
  onSuccess: async () => {
    showSuccessToast({ message: "提交成功", duration: 3000, position: "top" });
...
>({
  ...
  onSuccess: async () => {
    showSuccessToast({ message: "取消提交成功", duration: 3000, position: "top" });
    await refreshTeamData();
  },
  onError: (error) => {
...
>({
  ...
  onSuccess: async () => {
    showSuccessToast({ message: "解散成功", duration: 3000, position: "top" });
...
>({
  ...
  onSuccess: async () => {
    showSuccessToast({ message: "删除成功", duration: 3000, position: "top" });
...
>({
  ...
  onSuccess: async () => {
    showSuccessToast({ message: "移交成功", duration: 3000, position: "top" });
    handleMemberPopupClose();
...
>({
  ...
  onSuccess: async () => {
    showSuccessToast({ message: "更新成功", duration: 3000, position: "top" });
...
const handleDetailClick = () => {
  currentView.value = "basic-detail";
};
