...
import { Simplify, UnionToIntersection, ValueOf } from "type-fest";
...
/** 行程段配置
 *
 * 数据源 @see WALK_POINT_ID_MAP
 *
 * 详细注释 @see WALK_SEGMENT_CONFIG */
...
const SEGMENT_KEY_MAP = (() => {
  ...
  for (const routeId of WALK_ROUTE_ID_LIST) {
    ...
    // 从第二个开始遍历点位ID
    for (let i = 1; i < WALK_POINT_ID_MAP[routeId].length; ++i) {
      /** 上一点位ID */
      const previousPointId = WALK_POINT_ID_MAP[routeId][i - 1] as WalkPointId;
      /** 当前点位ID */
      const currentPointId = WALK_POINT_ID_MAP[routeId][i] as WalkPointId;
      keyArray.push(
...
/** 行程段key列表 */
export const WALK_SEGMENT_KEY_LIST = uniq(Object.values(SEGMENT_KEY_MAP).flat());

/** 行程段的导出量
 *
 * 数据源 @see WALK_POINT_ID_MAP
 * 数据源 @see WALK_POINT_CONFIG
 * 行程段的显示文本格式 @see SEGMENT_TEXT_DELIMITER
 *
 * 详细注释 @see WALK_SEGMENT_DERIVATIVE */
const SEGMENT_DERIVATIVE = (() => {
  ...
  for (const routeId of WALK_ROUTE_ID_LIST) {
    ...
    for (let i = 1; i < WALK_POINT_ID_MAP[routeId].length; ++i) {
      ...
      result[segmentKey] = {
        ...
        text: `${WALK_POINT_CONFIG[previousPointId].text}${SEGMENT_TEXT_DELIMITER}${WALK_POINT_CONFIG[currentPointId].text}`
      };
    }
  }
  return result;
 ...
 * 数据源 @see WALK_ROUTE_ID_MAP
 * 数据源 @see SEGMENT_KEY_MAP
 *
 * 详细注释 @see WALK_CAMPUS_SEGMENT_KEY_MAP */
const CAMPUS_SEGMENT_KEY_MAP = (() => {
  const result: Partial<Record<WalkCampusId, WalkSegmentKey[]>> = {};
  // 遍历所有校区ID
  for (const campusId of WALK_CAMPUS_ID_LIST) {
    // 计算点位ID
    result[campusId] = uniq(
      WALK_ROUTE_ID_MAP[campusId].flatMap((routeId) => SEGMENT_KEY_MAP[routeId])
    );
  }
  return result;
})() as unknown as WalkCampusSegmentKeyMap;
...
export interface WalkSegmentConfig {
  /** 行程段的点击热区HTML */
  hotSpotHTML?: Promise<string>; // TODO: 完成热区后将此字段改为必需
}
 ...
 *
 * @see WALK_POINT_ID_MAP 数据源 */
export type WalkSegmentKeyMap = {
  [RouteId in WalkRouteId]: SegmentKeyArrayBuilder<(typeof WALK_POINT_ID_MAP)[RouteId]>;
};
    ...
    [RouteId in WalkRouteId]: {
      ...
      `${infer From extends WalkPointId}${typeof SEGMENT_KEY_DELIMITER}${infer To extends WalkPointId}`
        ? {
            ...
            text: `${(typeof WALK_POINT_CONFIG)[From]["text"]}${typeof SEGMENT_TEXT_DELIMITER}${(typeof WALK_POINT_CONFIG)[To]["text"]}`;
 ...
 * 数据源 @see WALK_ROUTE_ID_MAP
 * 数据源 @see SEGMENT_KEY_MAP */
export type WalkCampusSegmentKeyMap = {
  // 遍历所有 校区ID
  [CampusId in WalkCampusId]: {
    // 遍历 校区ID 对应的 路段ID列表，进而取得 路段ID 对应的 点位ID
    [RouteId in (typeof WALK_ROUTE_ID_MAP)[CampusId][number]]: (typeof SEGMENT_KEY_MAP)[RouteId][number];
  }[(typeof WALK_ROUTE_ID_MAP)[CampusId][number]];
};
 ...
 *
 * 修改 @see SEGMENT_CONFIG */
export const WALK_SEGMENT_CONFIG = SEGMENT_CONFIG as AllowAnyStringKey<typeof SEGMENT_CONFIG>;

/** 路线ID-行程段key列表 映射表
 *
 * @see SEGMENT_KEY_DELIMITER 关于key的格式
 *
 * 修改 @see SEGMENT_KEY_MAP */
export const WALK_SEGMENT_KEY_MAP = SEGMENT_KEY_MAP as AllowAnyStringKey<typeof SEGMENT_KEY_MAP>;

/** 行程段的导出量
 * @property {WalkPointId} from 行程段的始点ID
 * @property {WalkPointId} to 行程段的末点ID
 * @property {string} text 行程段的显示文本
 *
 * 实现 @see SEGMENT_DERIVATIVE */
export const WALK_SEGMENT_DERIVATIVE = SEGMENT_DERIVATIVE as AllowAnyStringKey<
  typeof SEGMENT_DERIVATIVE
>;

/** 校区ID-行程段key列表 映射表
 * @description 每个校区的行程段列表中没有重复
 *
 * 实现 @see CAMPUS_SEGMENT_KEY_MAP */
export const WALK_CAMPUS_SEGMENT_KEY_MAP = CAMPUS_SEGMENT_KEY_MAP as AllowAnyStringKey<
  typeof CAMPUS_SEGMENT_KEY_MAP
>;
